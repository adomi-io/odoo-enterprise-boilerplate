...
export TESTS_WORKDIR="${TESTS_WORKDIR:-${CALLER_DIR}}"
...
if [ "${TESTS_SKIP_BUILD}" != "true" ]; then
  ...
  echo "Skipping image build as TESTS_SKIP_BUILD is set to true."
fi
...
# Use first arg if passed, otherwise resolve relative to caller dir
CONFIG_FILE="${1:-${CALLER_DIR}/config/odoo.conf}"
...
# Print result and return the test status if failed
